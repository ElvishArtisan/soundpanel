 ...
 *
 * WaveFile provides an abstraction of a Microsoft RIFF-based WAV file.
...
class WaveFile
{
 public:
  ...
  /**
   * Create an WaveFile object.
   * @param file_name The name of the WAV file to load into the object.
   **/
  WaveFile(QString file_name="");

   /**
    * Destroy an WaveFile object.
    **/
  ~WaveFile();
   ...
   **/
   bool createWave(WaveData *data=NULL);
   ...
   **/
   bool openWave(WaveData *data=NULL);
   ...
   **/
   WaveFile::Encoding encoding() const;

  /**
   * Set the encoding type.
   * @param code The encoding to use.
   **/
   void setEncoding(WaveFile::Encoding code);
   ...
   * Values currently understood by WaveFile are:
  ...
  private:
   ...
   unsigned energy_ptr;
   int wave_id;
   WaveFile::Type wave_type;

   unsigned char *cook_buffer;
   int cook_buffer_size;
   WaveFile::Encoding cook_encoding;
