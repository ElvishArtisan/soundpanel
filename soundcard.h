 ...
 * Definitions
 */
#define SOUNDCARD_HPI_MAX_LEVEL 2400
#define SOUNDCARD_HPI_MIN_LEVEL 0
...
class SoundCard : public QObject
{
 ...
 public:
  ...
  SoundCard(QObject *parent=0,const char *name=0);
  ~SoundCard();
  Driver driver() const;
  ...
  const Information *getCardInfo(int card) const;
  ...
  void setClockSource(int card,SoundCard::ClockSource src);
  ...
  SoundCard::TunerBand tunerBand(int card,int port);
  void setTunerBand(int card,int port,SoundCard::TunerBand band);
  int tunerFrequency(int card,int port);
  void setTunerFrequency(int card,int port,int freq);
  bool tunerSubcarrier(int card,int port,SoundCard::Subcarrier sub);
  int tunerLowFrequency(int card,int port,SoundCard::TunerBand band);
  int tunerHighFrequency(int card,int port,SoundCard::TunerBand band);
  ...
  SoundCard::SourceNode getInputPortMux(int card,int port);
  bool setInputPortMux(int card,int port,SoundCard::SourceNode node);
  SoundCard::FadeProfile getFadeProfile() const;
  unsigned short getInputPortError(int card,int port);
  void setFadeProfile(SoundCard::FadeProfile profile);
 ...
 signals:
  ...
  void inputMode(int card,int stream,SoundCard::ChannelMode mode);
  void outputMode(int card,int stream,SoundCard::ChannelMode mode);
  void tunerSubcarrierChanged(SoundCard::Subcarrier car,bool state);
 ...
 public slots:
  ...
  void setInputMode(int card,int stream,SoundCard::ChannelMode mode);
  void setOutputMode(int card,int stream,SoundCard::ChannelMode mode);
 ...
 private:
  ...
  int card_quantity;
  QTimer *clock_timer;
  SoundCard::FadeProfile fade_type;
  ...
  short 
   ...
   Information hpi_info[HPI_MAX_ADAPTERS];
};
